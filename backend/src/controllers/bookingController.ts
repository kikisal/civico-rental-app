...
export const getRoomBookings = async (req: Request, res: Response) => {
	try {
		...
		if (roomId === undefined || year === undefined || month === undefined) {
			console.log("getRoomBookings(): Invalid request.");
			res.sendStatus(204);
			return;
		}
		
		const _res = await fetch(`${env.BOOKING_API_BACKEND}/api/room-bookings/${roomId}?year=${year}&month=${month}`, {method: "GET"});

		if (_res.status !== 200) {
			console.log("getRoomBookings(): Invalid backend response status: ", _res.status);
			res.sendStatus(204);
			return;
		}
		...
		console.log("getRoomBookings() Exception Caught: ", ex);
		res.sendStatus(400);
	}
};
...
export const checkConflict = async (req: Request, res: Response) => {
	try {
		...
		if (roomId === undefined || startDate === undefined || endDate === undefined) {
			console.log("checkConflict(): Invalid request.");
			res.sendStatus(204);
			return;
		}
		
		const _res = await fetch(`${env.BOOKING_API_BACKEND}/api/bookings/check-conflict`, {
			method: "POST",
			headers: {
		...
		if (_res.status !== 200) {
			console.log("checkConflict(): Invalid backend response status: ", _res.status);
			res.sendStatus(204);
			return;
		}
		...
		res.status(200);
		res.send({hasConflict: data.hasConflict});
	} catch(ex) {
		console.log("checkConflict() Exception Caught: ", ex);
		res.sendStatus(400);
	}
};
